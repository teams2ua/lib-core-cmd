...
AsioHttpClient::AsioHttpClient(const std::shared_ptr<AsioExecutionContext> &context): _context(context), _ssl_ctx(boost::asio::ssl::context::sslv23_client) {
    _context = context;
	...
	_ssl_ctx.add_certificate_authority(
		boost::asio::buffer(cert.data(), cert.size()), err);
	if (err)
		return;
...
void AsioHttpClient::execute(const std::shared_ptr<api::HttpRequest> &request) {
	auto r = std::make_shared<RequestResponse>(_context->_io_service, _ssl_ctx, request);
	r->execute();
}
